...
// 遍历所有的参考线，针对每一条参考线，做plan, 并计算line的cost：
Status LatticePlanner::Plan(const TrajectoryPoint& planning_start_point,
                            Frame* frame) {
  std::size_t success_line_count = 0;
  std::size_t index = 0;
  // obtain a reference line and transform it to the PathPoint format：简单转化，增加Ｓ值
...
Status LatticePlanner::PlanOnReferenceLine(
  ...
  // line.
  // compute the matched point of the init planning point on the reference line : 从refline上找到起始点
  ...
  // 撒点逻辑，生成纵向的一组F(T)=S方程和横向的F(S)=L方程
  ...
  while (trajectory_evaluator.has_more_trajectory_pairs()) {
    ...
    // 根据约束条件，单独评估两种方程，每一个的合理性；对于合理的L和S，组成pair，并评估他的cost； 遍历所有的方程，将结果放在一个有序的queue中
    // combine two 1d trajectories to one 2d trajectory
    // 按照cost顺序，对于每一个pair，进行合并combined_trajectory，合并到一个2维方程中； 对于是否碰撞和有效性等进行检测，如果这条曲线合适，则直接输出； 结果放在reference_line_info中
  ...
  if (num_lattice_traj > 0) {
    ...
    // 如果创建Traje失败，则使用 BackupTrajectoryGenerator
