...
bool DPRoadGraph::SamplePathWaypoints(
  ...
  const float step_length =                                                   // 步长
      common::math::Clamp(init_point.v() * kSamplePointLookForwardTime,       // 当前速度乘以4秒, 反应时间为4秒?
  ...
  if (!status->has_pull_over() && status->pull_over().in_pull_over()) {
    ...
    if (init_sl_point_.s() > start_point_sl.s()) {
      const auto &stop_point = status->pull_over().stop_point();    // 拉回来? 
  ...
  for (std::size_t i = 0; accumulated_s < total_length; ++i) {   // 开始迭代
    accumulated_s += level_distance;                             // 一个步长
    if (accumulated_s + level_distance / 2.0 > total_length) {   // 到了终点
      accumulated_s = total_length;
    }
    const float s = std::fmin(accumulated_s, total_length);      // 找个小的
    ...
    reference_line_.GetLaneWidth(s, &left_width, &right_width);   // 获取车道的宽度, 左边和右边的宽度
    ...
    const double delta_dl = 1.2 / 20.0;                           // 启发式的参数delta, 1.2/20的具体含义是什么啊?
    ...
    float kDefaultUnitL = kChangeLaneDeltaL / (num_sample_per_level - 1);    // l默认的单位
    ...
    float sample_right_boundary = -eff_right_width;            // 右边为什么是减
    ...
    if (reference_line_info_.IsChangeLanePath() ||
      ...
      sample_right_boundary = std::fmin(-eff_right_width, init_sl_point_.l());   // 右boundary的大小
    ...
    if (reference_line_info_.IsChangeLanePath() &&
      ...
      switch (sidepass_.type()) {
        case ObjectSidePass::LEFT: {
          sample_l.push_back(eff_left_width + config_.sidepass_distance());     // 只会执行这条语句
          break;
        }
      ...
      common::util::uniform_slice(sample_right_boundary, sample_left_boundary,  // 切片
