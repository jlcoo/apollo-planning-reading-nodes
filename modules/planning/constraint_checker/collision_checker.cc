...
void CollisionChecker::BuildPredictedEnvironment(
  ...
  while (relative_time < FLAGS_trajectory_time_length) {
    ...
    for (const Obstacle* obstacle : obstacles_considered) {   // trajectory
      ...
      box.LongitudinalExtend(2.0 * FLAGS_lon_collision_buffer);   // 纵向延伸, 纵向2m
      box.LateralExtend(2.0 * FLAGS_lat_collision_buffer);        // 横向延伸, 横向0.1m
      predicted_env.push_back(std::move(box));
    }
    predicted_bounding_rectangles_.push_back(std::move(predicted_env));
    relative_time += FLAGS_trajectory_time_resolution; // 0.1ms
  }
}

bool CollisionChecker::IsEgoVehicleInLane(
    const double ego_vehicle_s, const double ego_vehicle_d) {
  double left_width = FLAGS_default_reference_line_width * 0.5;     // 参考线的宽度
  double right_width = FLAGS_default_reference_line_width * 0.5;    // 右车道的宽度
  ptr_reference_line_info_->reference_line().GetLaneWidth(
      ego_vehicle_s, &left_width, &right_width);
  return ego_vehicle_d < left_width && ego_vehicle_d > -right_width;  // 对半分
}
