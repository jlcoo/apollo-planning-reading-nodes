...
namespace apollo {
namespace planning {
// reference line是路网结构中一个很重要的概念，绘制地图的时候先是画reference line，
...
ReferenceLine::ReferenceLine(const MapPath& hdmap_path)   // using MapPath = hdmap::Path， 利用using替代掉了typedef
  ...
  for (const auto& point : hdmap_path.path_points()) {
    ...
    reference_points_.emplace_back(    // // ReferencePoint继承于MapPathPoint : public common::math::Vec2d 
...
ReferencePoint ReferenceLine::GetReferencePoint(const double s) const {
  const auto& accumulated_s = map_path_.accumulated_s();   // 获得积分的S
  if (s < accumulated_s.front() - 1e-2) {          // 太小
    AWARN << "The requested s " << s << " < 0";
    return reference_points_.front();
  }
  if (s > accumulated_s.back() + 1e-2) {           // 太大
    AWARN << "The requested s " << s << " > reference line length "
          << accumulated_s.back();
    return reference_points_.back();
  }

  auto interpolate_index = map_path_.GetIndexFromS(s);   // 插值的点
...
bool ReferenceLine::GetLaneWidth(const double s, double* const lane_left_width,   // 输入为s, 输出为lane左边的宽度和lane右边的宽度
