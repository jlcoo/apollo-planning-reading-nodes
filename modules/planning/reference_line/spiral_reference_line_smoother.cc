...
bool SpiralReferenceLineSmoother::Smooth(std::vector<Eigen::Vector2d> point2d,    
                                         std::vector<double>* ptr_theta,    // 保存结果
  ...
  SpiralProblemInterface* ptop = new SpiralProblemInterface(point2d);    // SpiralProblemInterface(螺旋问题)
  ...
  // Create an instance of the IpoptApplication
  Ipopt::SmartPtr<Ipopt::IpoptApplication> app = IpoptApplicationFactory();    // 创建一个IpoptApplication的实例
  ...
  Ipopt::ApplicationReturnStatus status = app->Initialize();    // 初始化
  if (status != Ipopt::Solve_Succeeded) {
    ADEBUG << "*** Error during initialization!";
    return false;
  }

  status = app->OptimizeTNLP(problem);     // 优化
