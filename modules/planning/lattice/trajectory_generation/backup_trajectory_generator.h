...
class BackupTrajectoryGenerator {
 public:
  typedef std::pair<std::shared_ptr<Curve1d>, std::shared_ptr<Curve1d>>  // 轨道的点对, 一维的曲线
      Trajectory1dPair;
  typedef std::pair<Trajectory1dPair, double> PairCost;   // 点对的代价点对
  // 备份的轨迹生成器
 ...
 private:
  ...
  // 仿函数是我们自己实现的, 让仿函数支持适配器必须由std::binary_function派生出来
  struct CostComparator   // 结构体继承了一个函数对象(仿函数)
      : public std::binary_function<const Trajectory1dPair&,
                                    const Trajectory1dPair&, bool> {
    bool operator()(const Trajectory1dPair& left,
                    const Trajectory1dPair& right) const {    // 重载小括号操作符
      auto lon_left = left.first;       // 左侧
      auto lon_right = right.first;     // 右侧
