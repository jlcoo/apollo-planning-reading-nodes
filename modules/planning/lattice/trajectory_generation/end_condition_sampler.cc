...
std::vector<Condition> EndConditionSampler::SampleLatEndConditions() const {
  ...
  std::array<double, 3> end_d_candidates = {0.0, -0.5, 0.5};      // d 是什么?
  std::array<double, 4> end_s_candidates = {10.0, 20.0, 40.0, 80.0};  // s 又是什么
...
std::vector<Condition> EndConditionSampler::SampleLonEndConditionsForCruising(
  ...
  for (std::size_t i = 0; i + 1 < num_of_time_samples; ++i) {
    time_samples[i] = FLAGS_trajectory_time_length - i;   // FLAGS_trajectory_time_length 被定义为8
  }
  time_samples[num_of_time_samples - 1] = FLAGS_polynomial_minimal_param; // FLAGS_polynomial_minimal_param为0.01
