...
class PredictionQuerier {
 public:
  explicit PredictionQuerier(
      ...
      const std::shared_ptr<std::vector<common::PathPoint>>&  // 共享指针参考线
      ptr_reference_line);
 ...
 private:
  std::unordered_map<std::string, const Obstacle*> id_obstacle_map_;   // 障碍物id和map映射的

  std::vector<const Obstacle*> obstacles_;                    // 障碍物
