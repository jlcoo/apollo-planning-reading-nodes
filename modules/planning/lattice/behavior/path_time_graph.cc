...
using apollo::common::math::lerp;                  // linear interpolation(线性插值) between a and b
using apollo::common::math::Box2d;                 // 二维矩阵
using apollo::common::math::Polygon2d;             // 二维多边形
using apollo::common::math::PathMatcher;           // 路径匹配器
using apollo::common::PathPoint;                   // 路径点
using apollo::common::TrajectoryPoint;             // 轨迹点
using apollo::perception::PerceptionObstacle;      // 感知的障碍物

PathTimeGraph::PathTimeGraph(
    const std::vector<const Obstacle*>& obstacles,
    const std::vector<PathPoint>& discretized_ref_points,   // 离散的路网点
    const ReferenceLineInfo* ptr_reference_line_info,       // reference_line
    const double s_start, const double s_end,               // 距离的起点，终点
    const double t_start, const double t_end,               // 时间的起点和终点
...
SLBoundary PathTimeGraph::ComputeObstacleBoundary(
  ...
  SLBoundary sl_boundary;           // slboundary(frenet坐标系下的边框)
...
void PathTimeGraph::SetupObstacles(
  ...
  for (const Obstacle* obstacle : obstacles) {
    ...
    if (!obstacle->HasTrajectory()) {     // 根据障碍物有没有轨迹进行判断是静态障碍物还是动态障碍物
  ...
  std::sort(static_obs_sl_boundaries_.begin(), static_obs_sl_boundaries_.end(),
      [](const SLBoundary& sl0, const SLBoundary& sl1) {   // 降续排序
  ...
  for (auto& path_time_obstacle : path_time_obstacle_map_) {
    double s_upper = std::max(path_time_obstacle.second.bottom_right().s(),
                              path_time_obstacle.second.upper_right().s());  // s的上界
    double s_lower = std::min(path_time_obstacle.second.bottom_left().s(),
                              path_time_obstacle.second.upper_left().s());   // s的下届
    ...
    path_time_obstacles_.push_back(path_time_obstacle.second);   // 第一项是string
  }
}
...
void PathTimeGraph::SetStaticObstacle(
    ...
    const std::vector<PathPoint>& discretized_ref_points) {
  const Polygon2d& polygon = obstacle->PerceptionPolygon();   // 感知的多边形
...
PathTimeGraph::GetPathBlockingIntervals(const double t_start,
  ...
  return intervals;   // 得到一个时间段的vector
}
...
std::vector<std::pair<double, double>> PathTimeGraph::GetLateralBounds(
  ...
  std::vector<double> discretized_path;    // 离散的路径
  double s_range = s_end - s_start;
  double s_curr = s_start;
  std::size_t num_bound = static_cast<std::size_t>(s_range / s_resolution);  // bound是界

  const auto& vehicle_config =
      common::VehicleConfigHelper::instance()->GetConfig();   // 单例
