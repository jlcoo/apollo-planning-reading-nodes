...
class IndexedList {   // 保存索引的容器, 拷贝目标到容器中, 如果id已经存在了, 会复写
 public:
  /**
 ...
 private:
  std::vector<const T*> object_list_;         // 用了一个vector和hash表实现的
  std::unordered_map<I, T> object_dict_;      // 用map来保存目标的字典
};

template <typename I, typename T>       // 索引
class ThreadSafeIndexedList : public IndexedList<I, T> {
 public:
  T* Add(const I id, const T& object) {
    boost::unique_lock<boost::shared_mutex> writer_lock(mutex_);   // 加写锁
    return IndexedList<I, T>::Add(id, object);
  }

  T* Find(const I id) {
    boost::shared_lock<boost::shared_mutex> reader_lock(mutex_);   // 加读锁
