...
TEST(StBoundaryTest, basic_test) {
  ...
  EXPECT_FLOAT_EQ(0.0, boundary.min_s());   // s是宽度？
  EXPECT_FLOAT_EQ(5.0, boundary.max_s());
  EXPECT_FLOAT_EQ(0.0, boundary.min_t());   // t是长度？
...
TEST(StBoundaryTest, boundary_range) {
  ...
  boundary.SetBoundaryType(StBoundary::BoundaryType::YIELD);  // 让行
  double t = -10.0;    
...
TEST(StBoundaryTest, remove_redundant_points) {
  ...
  st_boundary.RemoveRedundantPoints(&points);    // 其实是扩大boundary
