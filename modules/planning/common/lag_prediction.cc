...
using apollo::perception::PerceptionObstacle;                   // 感知障碍物
using apollo::prediction::PredictionObstacle;                   // 预测障碍物
using apollo::prediction::PredictionObstacles;                  // 预测多个障碍物

LagPrediction::LagPrediction(uint32_t min_appear_num,           // 最小的数量
...
void LagPrediction::GetLaggedPrediction(PredictionObstacles* obstacles) const {
  ...
  const auto adc_position =     // 获得最近的障碍物的位置信息
  ...
  std::unordered_set<int> protected_obstacles;     // 受保护的障碍物
  for (const auto& obstacle : latest_prediction->prediction_obstacle()) {
    ...
    if (perception.confidence() < FLAGS_perception_confidence_threshold &&
        perception.type() != PerceptionObstacle::VEHICLE) {   // 置信度太低的话直接越过
      continue;
    }
    double distance =
  ...
  std::unordered_map<int, LagInfo> obstacle_lag_info;    // 障碍物的lag误差
