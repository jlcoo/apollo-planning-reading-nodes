...
Obstacle::Obstacle(const std::string& id,
  ...
  for (int i = 1; i < trajectory_points.size(); ++i) {
    ...
    trajectory_points[i].mutable_path_point()->set_s(cumulative_s);  // 累加的距离
  }
}
...
bool Obstacle::IsStaticObstacle(const PerceptionObstacle& perception_obstacle) {
  if (perception_obstacle.type() == PerceptionObstacle::UNKNOWN_UNMOVABLE) {  // 有明确的标注
    return true;
  }   // 没有标注的话, 应该有速度的限制, 速度低于多少才是静止障碍物
...
common::TrajectoryPoint Obstacle::GetPointAtTime(                                // 通过一个相对时间获得一个轨迹点
  ...
  if (points.size() < 2) {
    ...
    auto it_lower =
        std::lower_bound(points.begin(), points.end(), relative_time, comp);  // 小于relative_time的第一个
...
std::list<std::unique_ptr<Obstacle>> Obstacle::CreateObstacles(    // 创建障碍物
  ...
  for (const auto& prediction_obstacle : predictions.prediction_obstacle()) {
    ...
    for (const auto& trajectory : prediction_obstacle.trajectory()) {
      ...
      const std::string obstacle_id =
          apollo::common::util::StrCat(perception_id, "_", trajectory_index);   // 感知的物体
      obstacles.emplace_back(new Obstacle(
...
std::unique_ptr<Obstacle> Obstacle::CreateStaticVirtualObstacles(
    const std::string& id, const common::math::Box2d& obstacle_box) {    // 创建一个虚拟障碍物
