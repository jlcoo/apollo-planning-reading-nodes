...
using apollo::perception::PerceptionObstacle;   // 使用感知的命名空间

TEST(Obstacle, IsStaticObstacle) {
  PerceptionObstacle perception_obstacle;
  EXPECT_TRUE(Obstacle::IsStaticObstacle(perception_obstacle));   // 静态障碍物

  perception_obstacle.mutable_velocity()->set_x(2.5);             // 宽2.5m
  perception_obstacle.mutable_velocity()->set_y(0.5);             // 长0.5m
  EXPECT_FALSE(Obstacle::IsStaticObstacle(perception_obstacle));

  perception_obstacle.set_type(PerceptionObstacle::UNKNOWN);
  EXPECT_FALSE(Obstacle::IsStaticObstacle(perception_obstacle));

  perception_obstacle.set_type(PerceptionObstacle::UNKNOWN_UNMOVABLE);   // 没有移动
...
TEST_F(ObstacleTest, CreateObstacles) {
  ...
  EXPECT_TRUE(indexed_obstacles_.Find("2156_0"));    // 障碍物的ID号
...
TEST_F(ObstacleTest, GetPointAtTime) {
  ...
  // last
  const auto last_point = obstacle->GetPointAtTime(10.04415320);  // 相对时间
  ...
  // middle
  const auto middle_point = obstacle->GetPointAtTime(3.7300);  // 随便get一个中间点
...
TEST(Obstacle, CreateStaticVirtualObstacle) {
  ...
  EXPECT_EQ("abc", obstacle->Id());   // abc的id
  EXPECT_EQ(-314721735, obstacle->PerceptionId());
  EXPECT_TRUE(Obstacle::IsStaticObstacle(obstacle->Perception()));    // 是静态障碍物
  EXPECT_TRUE(Obstacle::IsVirtualObstacle(obstacle->Perception()));   // 是虚拟障碍物
  auto& perception_box = obstacle->PerceptionBoundingBox();           // 感知到的边框
  EXPECT_FLOAT_EQ(0.0, perception_box.center().x());                  // 中心点
