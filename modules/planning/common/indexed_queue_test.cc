...
using StringIndexedQueue = IndexedQueue<int, std::string>;   // 带索引的队列
TEST(IndexedQueue, QueueSize1) {
  StringIndexedQueue object(1);    // 队列的长度为1
  ...
  ASSERT_FALSE(object.Add(1, std::make_unique<std::string>("one")));  // 不相等
...
TEST(IndexedQueue, QueueSize2) {
  StringIndexedQueue object(2);    // 队列的长度为2
  ASSERT_TRUE(object.Add(1, std::make_unique<std::string>("one")));
  ASSERT_TRUE(object.Find(1) != nullptr);
  ASSERT_TRUE(object.Find(2) == nullptr);
  ...
  ASSERT_TRUE(object.Find(1) != nullptr);
  ASSERT_TRUE(object.Find(2) != nullptr);
  ASSERT_EQ("two", *object.Latest());     // Latest得到的是最近加入队列的对象
